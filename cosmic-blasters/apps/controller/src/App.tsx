...
import { ensureLocalHubSessionId, GAME_CONSTANTS } from "@cosmic-blasters/shared"
...
function MaybePlatformProvider({ children }: { children: ReactNode }) {
    ...
    try {
        ...
        console.warn("[CosmicBlasters] PlatformProvider failed to load, running without it:", err)
        return <>{children}</>
    }
}
    ...
    if (!sessionId) {
        return (
            <div style={{ padding: 32, fontFamily: "sans-serif", textAlign: "center" }}>
                <h1>Cosmic Blasters Controller</h1>
